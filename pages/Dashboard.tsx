    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <section className="flex flex-col md:flex-row justify-between items-end gap-6">
        ...
        <div className="flex gap-3">
            <button className="flex items-center gap-2 px-5 py-3 bg-white/60 backdrop-blur-md text-slate-700 font-bold rounded-full border border-white hover:bg-white hover:text-brand-600 transition-all shadow-sm hover:shadow-md active:scale-95">
              <Calendar className="w-4 h-4" />
              This Month
            </button>
            <button className="flex items-center gap-2 px-6 py-3 bg-brand-500 text-white font-bold rounded-full hover:bg-brand-600 transition-all shadow-lg shadow-brand-500/30 hover:shadow-brand-500/40 active:scale-95">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {/* Main Chart */}
        <div className="xl:col-span-2 glass-panel p-8 rounded-3xl animate-slide-up hover:border-white/80 transition-colors" style={{ animationDelay: '400ms' }}>
          <div className="flex justify-between items-center mb-8">
            <div>
              <h3 className="text-xl font-bold text-slate-800">Spending Analysis</h3>
              <p className="text-sm text-slate-500">Your spending trend over time</p>
            </div>
            <div className="flex bg-slate-100/50 p-1 rounded-xl">
              {['Week', 'Month', 'Year'].map(period => (
                <button key={period} className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-all ${period === 'Week' ? 'bg-white shadow-sm text-brand-600' : 'text-slate-500 hover:text-slate-700'}`}>
                  {period}
                </button>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel p-8 rounded-3xl animate-slide-up flex flex-col justify-center min-h-[350px]" style={{ animationDelay: '500ms' }}>
            <h3 className="text-xl font-bold text-slate-800 mb-2">Expenses by Category</h3>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
        ...
        <div className="glass-panel p-8 rounded-3xl animate-slide-up" style={{ animationDelay: '700ms' }}>
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="text-brand-600 text-sm font-bold hover:text-brand-700 flex items-center gap-1 px-3 py-1.5 rounded-lg hover:bg-brand-50 transition-colors">
              View All <ArrowRight className="w-4 h-4" />
            </button>
          </div>
          <div className="space-y-2">
            {RECENT_TRANSACTIONS.map((t) => (
              <div key={t.id} className="flex items-center justify-between p-4 rounded-2xl hover:bg-white/40 transition-all group cursor-pointer border border-transparent hover:border-white/50">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors shadow-sm ${
                    t.type === 'expense' ? 'bg-rose-50 text-rose-500 group-hover:bg-rose-100' : 'bg-emerald-50 text-emerald-500 group-hover:bg-emerald-100'
                  }`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-slate-400 mt-0.5">{t.date} • {t.category}</p>
                  </div>
                </div>
        ...
        <div className="glass-panel p-8 rounded-3xl animate-slide-up" style={{ animationDelay: '800ms' }}>
            <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-slate-800">Upcoming Tasks</h3>
          ...
          <div className="space-y-3">
            {UPCOMING_TASKS.map((task) => (
              <div key={task.id} className="flex items-center gap-4 p-4 border border-white/40 rounded-2xl bg-white/20 hover:bg-white/60 transition-all hover:shadow-md group cursor-pointer">
                  <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center cursor-pointer transition-all ${
                    task.status === 'completed' ? 'bg-brand-500 border-brand-500' : 'border-slate-300 hover:border-brand-400'
                  }`}>
                    {task.status === 'completed' && <div className="w-3 h-3 bg-white rounded-sm" />}
                  </div>
                  <div className="flex-1">
                    <p className="font-semibold text-slate-700 group-hover:text-slate-900 transition-colors text-base">
                      {task.title}
                    </p>
            ...
            <button className="w-full py-4 border-2 border-dashed border-slate-300/50 rounded-2xl text-slate-400 text-sm font-bold hover:border-brand-300 hover:text-brand-500 hover:bg-brand-50/50 transition-all flex items-center justify-center gap-2 mt-4">
