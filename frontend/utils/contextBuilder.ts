...
export const buildFinancialContext = (
  ...
  const avgDailySpend = daysPassed > 0 ? metrics.spentToday / 1 : 0; // Simplified for "today", ideally we need monthly total / days
  // Better approximation if we sum up this month's expenses from transactions list
  ...
  // Look for transactions with same title and amount happening more than once
  // This is a naive implementation for context only
